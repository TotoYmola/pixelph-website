...
async function decide(id,status,button){
  ...
  try{
    ...
    msg.className='notice admin-flash';msg.textContent=(status==='approved'?'Application approved.':'Application rejected.')+(j.warnings?.length?` Discord sync note: ${j.warnings.join(' | ')}`:'');msg.classList.remove('hidden');
